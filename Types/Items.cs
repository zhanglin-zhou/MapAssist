...
namespace MapAssist.Types
{
    ...
    class Items
    {
        ...
        public static void ResetForId(int id)
        {
            if (!ItemUnitHashesSeen.TryGetValue(id, out var _))
            {
                ItemUnitHashesSeen.Add(id, new HashSet<string>());
                ItemUnitIdsSeen.Add(id, new HashSet<uint>());
                ItemLog.Add(id, new List<UnitAny>());
            }
            else
            {
                ItemUnitHashesSeen[id].Clear();
                ItemUnitIdsSeen[id].Clear();
                ItemLog[id].Clear();
            }
        }

        public static void AddItem(int id, Types.UnitAny unitAny)
        {
            if (!ItemUnitHashesSeen[id].Contains(unitAny.ItemHash()) &&
                 !ItemUnitIdsSeen[id].Contains(unitAny.UnitId) && 
                 LootFilter.Filter(unitAny))
            {
                if (MapAssistConfiguration.Loaded.ItemLog.PlaySoundOnDrop) AudioPlayer.PlayItemAlert();

                ItemUnitHashesSeen[id].Add(unitAny.ItemHash());
                ItemUnitIdsSeen[id].Add(unitAny.UnitId);
                if (ItemLog[id].Count == MapAssistConfiguration.Loaded.ItemLog.MaxSize)
                {
                    ItemLog[id].RemoveAt(0);
                    ItemLog[id].Add(unitAny);
                }
                else
                {
                    ItemLog[id].Add(unitAny);
